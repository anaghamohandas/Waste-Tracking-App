...
export default function ReportPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const text = await response.text(); // ✅ Use only once
    ...
    try {
      ...
      // Optional: Trigger rewards refresh if your component supports it
...
useEffect(() => {
  const fetchUserAndReports = async () => {
    if (clerkUser && clerkUser.emailAddresses.length > 0) {
      ...
      const name = clerkUser.fullName || 'Anonymous User';

      let dbUser = await getUserByEmail(email);
      if (!dbUser) {
        dbUser = await createUser(email, name);
      }
      setUser(dbUser);
