...
import { getLeaderboardRewards, getUserByEmail } from '@/utils/db/actions'
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const fetchedRewards = await getLeaderboardRewards()
        setRewards(fetchedRewards)

        if (clerkLoaded && clerkUser?.primaryEmailAddress?.emailAddress) {
          ...
          toast.error('Please log in to see your rank.')
        }
      } catch (error) {
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="text-sm font-semibold text-gray-900">{typeof reward.points === 'number' ? reward.points.toLocaleString() : '—'}
</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
