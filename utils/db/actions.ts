...
import { ReportInsert } from './schema';
...
export async function createReport(
  ...
  try {
    const reportData: ReportInsert = {
      ...
      location,
      wasteType,
      amount: amount.toString(), // because your schema uses varchar for amount
      imageUrl,
      verificationResult,
      status: "pending",
    };
    ...
    // Continue with rewards/transactions...
    await updateRewardPoints(userId, 10);
    await createTransaction(userId, 'earned_report', 10, 'Points earned for reporting waste');
    await createNotification(userId, `You've earned 10 points for reporting waste!`, 'reward');
